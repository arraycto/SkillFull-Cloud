...
// +----------------------------------------------------------------------
package indi.zxiaozhou.skillfull.auth.security.authorization.picture;
...
public class PictureAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (StringUtils.isBlank(model.getCodeId())) {
            throw new BadCredentialsException("系统标识不能为空");
        }
        ...
        UserAndAuthModel userAndAuthModel = userDetails.getUserAndAuthModel();
        // 验证用户登录次数
        String failMsg = "登录失败,密码不正确";
        if (properties.isOpenAuthLoginErrorLimit() && properties.getMaxLoginErrorNum() > 0) {
            if (userAndAuthModel.getLoginFailErrorNum() > properties.getMaxLoginErrorNum()) {
                throw new BadCredentialsException("密码错误次数过多,账号已经锁定,请通知管理员或使用短信验证码登录");
            }
            failMsg += ",剩余" + (properties.getMaxLoginErrorNum() - userAndAuthModel.getLoginFailErrorNum() - 1) + "次重试机会";
        }
        RbacUserEntity rbacUserEntity = new RbacUserEntity();
        rbacUserEntity.setUserId(userAndAuthModel.getUserId());
        rbacUserEntity.setUpdateUserName(userAndAuthModel.getRealName());
        rbacUserEntity.setUpdateUserId(userAndAuthModel.getUserId());
        rbacUserEntity.setCurrentLoginTime(LocalDateTime.now());
        if (!CryptAuthUtils.matches(password, userAndAuthModel.getSalt(), userDetails.getPassword())) {
            // 设置登录信息
            rbacUserEntity.setLoginFailErrorNum(userAndAuthModel.getLoginFailErrorNum() + 1);
            boolean b = loginService.updateById(rbacUserEntity);
            if (!b) {
