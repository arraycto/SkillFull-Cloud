    ...
    public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        if (StringUtils.isNotBlank(token)) {
            try {
                ...
                tokenStore.removeToken(userTokenSubInfoModel.getSystemId(), userTokenSubInfoModel.getUserId());
            } catch (Exception e) {
                log.error("------------CustomLogoutHandler------退出登录失败------>logout:{},authentication:{}", e.getMessage(), authentication);
            }
        }
    }
}
