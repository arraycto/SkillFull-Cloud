...
public class ServletUtils {
    ...
    public static String getToken(HttpServletRequest request) {
        ...
        if (Objects.nonNull(systemBaseInfo)) {
            String header = request.getHeader(systemBaseInfo.getTokenHeaderKey());
            if (StringUtils.isNotBlank(header)) {
                if (StringUtils.isNotBlank(systemBaseInfo.getTokenHeaderStartWith())) {
                    header = header.replaceFirst(systemBaseInfo.getTokenHeaderStartWith(), "");
                }
                return header;
            } else {
                return request.getParameter(systemBaseInfo.getTokenQueryKey());
            }
        }
        return "";
    }
    ...
    public static String getToken(Session session) {
        ...
        if (Objects.nonNull(systemBaseInfo)) {
            ...
            Map<String, String> paramMap = CoreCommonUtils.getQueryMap(queryString);
            if (CollectionUtil.isNotEmpty(paramMap)) {
                return paramMap.get(systemBaseInfo.getTokenQueryKey());
            }
        }
        return "";
    }
    ...
    public static boolean isAjaxRequest(HttpServletRequest request) {
        if (Objects.nonNull(request)) {
            ...
            if (StringUtils.isBlank(accept) || !accept.contains(MediaType.APPLICATION_JSON_VALUE)) {
