...
@Slf4j
@Component
public class TokenRefreshGatewayFilterFactory extends AbstractGatewayFilterFactory<TokenRefreshGatewayFilterFactory.Config> {
    ...
    public TokenRefreshGatewayFilterFactory() {
        super(TokenRefreshGatewayFilterFactory.Config.class);
    }
    ...
    private class AuthorizeGatewayFilter implements GatewayFilter, Ordered {
        ...
        private GatewayFilterFactory<TokenRefreshGatewayFilterFactory.Config> gatewayFilterFactory;
        ...
        public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
            ServerHttpResponseDecorator response = new ServerHttpResponseDecorator(exchange.getResponse()) {
                ...
                public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
                    DataBufferFactory bufferFactory = this.getDelegate().bufferFactory();
                    ...
                    if (Objects.equals(getStatusCode(), HttpStatus.OK) && body instanceof Flux) {
                        ...
                        return super.writeWith(fluxBody.buffer().map(dataBuffers -> {//解决返回体分段传输
                            ...
                            this.getDelegate().getHeaders().setContentLength(uppedContent.length);
        ...
        public void setFactory(GatewayFilterFactory<TokenRefreshGatewayFilterFactory.Config> gatewayFilterFactory) {
