...
public class ManageAnnouncementVo implements Serializable {
    ...
    @ApiModelProperty(name = "receiveType", value = "接收对象类型：1-指定用户,2-指定区域,3-指定组织,4-指定角色,5-所有", required = true)
    @NotBlankOrNull(message = "接收对象类型：1-指定用户,2-指定区域,3-指定组织,4-指定角色,5-所有不能为空")
    private Integer receiveType;

    @ApiModelProperty(name = "receiveInfos", value = "接收对象", required = true)
    @NotBlankOrNull(message = "接收对象不能为空")
    private String receiveInfos;

    @ApiModelProperty(name = "sendType", value = "发布方式：0-手动,1-自动", required = true)
    @NotBlankOrNull(message = "发布方式：0-手动,1-自动不能为空")
    private Integer sendType;

    @ApiModelProperty(name = "finalySendTime", value = "最终发布时间", required = true)
    ...
    @ApiModelProperty(name = "cancelTime", value = "撤销时间", required = true)
    @NotBlankOrNull(message = "撤销时间不能为空")
    private LocalDateTime cancelTime;

    @ApiModelProperty(name = "sendStatus", value = "发布状态：0未发布，1已发布，2已撤销，默认0", required = true)
